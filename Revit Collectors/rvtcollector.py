...
if __name__ == "__main__":
    ...
    while run:
        ...
        for file_dir in file_dirs:
            ...
            print(f'Please wait while searching for Revit models.')
                ...
                for file in f:
                    if str(file).endswith(file_type):
                        ...
                        if len(long_name) > 259:
                            ...
                            pass
                        rvt_files[os.path.join(r, file)] = file

        print(f'{len(rvt_files)} total Revit models collected.')
        ...
        if not os.path.exists(rvt_paths_csv):
            try:
                #write collected paths to new csv logfile
                with open(rvt_paths_csv, "w", newline = '', encoding="utf-8") as log:
                    log_write = csv.writer(log, delimiter = ",", quotechar = '"', quoting = csv.QUOTE_MINIMAL)
                    log_write.writerow(['File Name', 'Path'])
                    for a,b in rvt_files.items():
                        log_write.writerow([b,a])

                print(f'All Revit model paths successfully logged.')
                print(f'See {rvt_paths_csv} for details')
                break
        ...
        #if log file exists 
        elif os.path.exists(rvt_paths_csv):
            print("exists")
            try:
                #add collected paths to existing csv logfile
                with open(rvt_paths_csv, "a", newline = '', encoding="utf-8") as log:
                    log_write = csv.writer(log, delimiter = ",", quotechar = '"', quoting = csv.QUOTE_MINIMAL)
                    for a,b in rvt_files.items():
                        log_write.writerow([b,a])
